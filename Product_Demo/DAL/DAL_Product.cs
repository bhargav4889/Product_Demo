...
namespace Product_Demo.DAL
{
    public class DAL_Product : DAL_Connection
    {
        ...
        public bool Update_Product(Product_Model product)
        {
            try
            {
                string imagePath = product.Product_Image != null
                       ? SaveProductImage(product.Product_Image, product.Product_Name)
                       : product.Product_Image_Path; // Retain old path if no new image
                ...
                using (SqlConnection sqlConnection = new SqlConnection(GetDatabaseConnection(_configuration)))
                {
                    ...
                    sqlCommand.Parameters.AddWithValue("@Product_Price", product.Product_Price);
                    sqlCommand.Parameters.AddWithValue("@Product_Status", product.Product_IsActive);
                    sqlCommand.Parameters.AddWithValue("@Product_Image_Path", string.IsNullOrEmpty(imagePath) ? (object)DBNull.Value : imagePath); // Allow null for image path
        ...
        public string SaveProductImage(IFormFile file, string productName)
        {
            if (file == null)
            {
                return null; // Return null or an appropriate default value if no file is provided
            }
